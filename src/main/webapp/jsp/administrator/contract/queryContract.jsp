        ...
        <div class="layui-colla-item">
            ...
            <div class="layui-colla-content">
                ...
                <p>
                    ...
                    到房屋所在地的房屋所在地的房屋行政管理部门办理登记变更、注销手续。在本合同有效期内，居住人员发生变更的，乙方应当自变更之日起2日内告知该备案机构
                    ，办理变更登记手续。<br>
        ...
        <div class="layui-colla-item">
            ...
            <div class="layui-colla-content">
                ...
                <p>
                    ...
                    &emsp;&emsp;6、隐瞒房屋抵押状况、产权转移情况或其他权利瑕疵，应向乙方使用的。<br>
...
<script type="text/html" id="queryAllContractInfoListBar">
    ...
    <a class="layui-btn layui-btn layui-btn-xs" lay-event="export">导出预览</a>
...
<script id="contractPrintPreview" type="text/html">
    <xdoc version="12.1.7">
        ...
        <para align="center" lineSpacing="4">
            <text fontStyle="bold" fontSize="21" >房屋租赁合同</text>
        </para>
        <para lineSpacing="4"/>
        <para lineSpacing="4">
            <text>●出租人（甲方）：</text>
            <text fontStyle="underline"> {{d.partAName}} </text>
            <text>  ●证件类型：</text>
            <text fontStyle="underline"> {{d.partATypeOfCertificate}} </text>
            <text>  ●证件编号：</text>
...
<script>
    ...
    layui.use(['element','form','jquery','table','laytpl'], function(){
        ...
        table.on('tool(queryAllContractInfoList)', function(obj){
            ...
            //导出事件
            if (obj.event === 'export'){
                viewExport(data.id, data.contractNumber, data.houseId);
            }
        ...
        function showContractMsg(id) {
            $.ajax({
                ...
                ,success:function (data) {
                    ...
                    var data2 = {
                        "viewPartAName": data[0].partAName,
                        "viewPartATypeOfCertificate" : data[0].viewPartATypeOfCertificate
                    }
                }
            })
        }

        function viewExport(id, contractNumber, houseId){
            previewHouseMsg(houseId);
            previewContractMsg(id);


        }

        function previewHouseMsg(houseId){



        }

        function previewContractMsg(id){
            $.ajax({
                url:'${ctx}/contract/viewContractMsg/'+ id
                ,type:'post'
                ,success:function (msg) {
                    ...
                    var data = {
                        "partAName": msg[0].partAName,
                        "partATypeOfCertificate" : msg[0].partATypeOfCertificate,
                        "partAIdNumber": msg[0].partAIdNumber
                    };
